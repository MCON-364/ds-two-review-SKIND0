...
public class TicketReportBuilder {
    ...
    public double getAverageResolutionMinutes() {
        // TODO: calculate from tickets
        return tickets.stream().filter(SupportTicket::resolved).mapToInt(SupportTicket::minutesToResolve).average().orElse(0.0);
    }
    ...
    public Map<String, Long> getCountByCategory() {
        // TODO: calculate from tickets
        return Map.copyOf(tickets.stream().collect(Collectors
                .groupingBy(SupportTicket::category, Collectors.counting())));
    }
    ...
    public List<SupportTicket> getHighPriorityUnresolved() {
        // TODO: calculate from tickets
        return tickets.stream().filter(t -> t.priority() == Priority.HIGH).filter(t -> !t.resolved()).toList();
    }
